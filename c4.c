...
// tweaked by Derek Denny-Brown to run with long=64bit
...
enum {
  ...
  Char, Else, Enum, If, Int, Long, Return, Sizeof, While,
...
void next()
{
  ...
  while ((tk = *p)) {
    ...
    if (tk == '\n') {
      ...
      id = sym;
      while (id[Tk]) {
        if (tk == id[Hash] && !memcmp((char *)id[Name], pp, p - pp)) { tk = id[Tk]; tk=(Long==tk)?Int:tk; return; }
        id = id + Idsz;
      }
      id[Name] = (long)pp;
      id[Hash] = tk;
...
int main(int argc, char **argv)
{
  ...
  if (!(sym = malloc((int)poolsz))) { printf("could not malloc(%d) symbol area\n", poolsz); return -1; }
  if (!(le = e = malloc((int)poolsz))) { printf("could not malloc(%d) text area\n", poolsz); return -1; }
  if (!(data = malloc((int)poolsz))) { printf("could not malloc(%d) data area\n", poolsz); return -1; }
  if (!(sp = malloc((int)poolsz))) { printf("could not malloc(%d) stack area\n", poolsz); return -1; }

  memset(sym,  0, poolsz);
  memset(e,    0, poolsz);
  memset(data, 0, poolsz);

  p = "char else enum if int long return sizeof while "
  ...
  *--sp = (long)t;
  ...
  while (1) {
    ...
    else if (i == LI)  a = *(long *)a;                                     // load int
    else if (i == LC)  a = *(char *)a;                                    // load char
    else if (i == SI)  *(long *)*sp++ = a;                                 // store int
